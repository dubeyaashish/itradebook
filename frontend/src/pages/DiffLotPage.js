    ...
    try {
      ...
      setSeriesMap(() => {
        ...
        groups.forEach(g => { next[g.symbol_ref] = {
          vals: (g.points || []).map(p => Number(p.difflot) || 0),
      ...
      const synth = groups.map(g => {
        const arr = g.points || [];
        const second = arr[arr.length - 2] || arr[arr.length - 1] || { difflot: 0 };
        return { symbol_ref: g.symbol_ref, difflot: second.difflot };
      });
      if (synth.length) setRows(synth);
    } catch (e) {
      if (e?.code === 'ERR_CANCELED' || e?.message === 'canceled') {
        console.debug('[DiffLot] fetch canceled');
        return;
      }
      console.error('[DiffLot] series fetch error:', e?.message);
      ...
      return (
        <div key={r.symbol_ref} className="metric-card">
          <SparklineCard
            ...
            timestamps={ts}
            value={val}
            fmt={(n) => Number(n).toLocaleString('en-US', { maximumFractionDigits: 4 })}
            positiveIsGood={true}
          />
        </div>
      );
  ...
  return (
    <div className="page-container" style={{ padding: 16 }}>
      <div className="data-container" style={{ background: 'transparent' }}>
        <DashboardHeader title="Diff Lot" subtitle="Live, second latest per symbol" pollingSec={POLL_MS/1000} />
        <MetricFilterBar
