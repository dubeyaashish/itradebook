// frontend/src/pages/EodBalancePage.js
...
const EodBalancePage = () => {
  ...
  const fetchSymbols = useCallback(async () => {
    try {
      ...
      const response = await axios.get('/api/eod-balance/symbols', {
  ...
  const fetchData = useCallback(async () => {
    try {
      ...
      const response = await axios.get(`/api/eod-balance?${params}`, {
        headers: { Authorization: `Bearer ${token}` }
      });
      ...
      if (response.data.success) {
        ...
        setError('Failed to fetch EOD balance data');
      }
    } catch (error) {
      console.error('Error fetching data:', error);
      setError(error.response?.data?.error || 'Failed to fetch EOD balance data');
    } finally {
      setLoading(false);
    }
  ...
  const handleExport = async () => {
    try {
      ...
      const response = await axios.get(`/api/eod-balance/export?${params}`, {
        headers: { Authorization: `Bearer ${token}` },
        responseType: 'blob'
      });
      ...
      link.download = `eod_balance_${timestamp}.csv`;
  ...
  if (loading) {
    return (
      <div className="page-container">
        <PageHeader 
          title="EOD - Balance" 
          subtitle="End-of-day accumulator balance data"
        />
        <div className="loading-container">
          <div className="loading-spinner">
            <div className="spinner"></div>
            <p>Loading EOD balance data...</p>
          </div>
        </div>
      </div>
    );
      ...
      <div className="filter-container">
        ...
        <div className="data-container">
          <div className="table-header">
            <h3>EOD Balance Data ({formatNumber(pagination.total, 0)} records)</h3>
          </div>        {data.length === 0 ? (
          <div className="no-data-message">
            No EOD balance data found for the selected filters.
          </div>
        ) : (
          <div className="table-responsive">
            <table className="data-table">
              ...
              <tbody>
                {data.map((row, index) => (
                  <tr key={`${row.symbol_ref}-${row.day_key}-${index}`}>
                    <td className="font-semibold text-blue-600">
                      {row.symbol_ref}
                    </td>
                    <td className="font-mono">
                      {formatDate(row.day_key)}
                    </td>
                    <td className="text-right font-mono">
                      <span className="text-green-600 font-semibold">
                        {formatNumber(row.total_deposit)}
                      </span>
                    </td>
                    <td className="text-right font-mono">
                      <span className="text-red-600 font-semibold">
                        {formatNumber(row.total_withdrawal)}
                      </span>
                    </td>
                    <td className="text-right font-mono">
                    ...
                    <td className="font-mono text-sm text-gray-600">
                      {row.last_second_id || 'N/A'}
                    </td>
                    <td className="font-mono text-sm text-gray-600">
                      {row.last_third_id || 'N/A'}
                    </td>
                    <td className="font-mono text-sm text-gray-600">
            ...
            <div className="grid grid-cols-1 md:grid-cols-4 gap-4 text-sm">
              ...
              <div>
                ...
                <span className="ml-2 font-semibold text-green-600">
                  {formatNumber(data.reduce((sum, row) => sum + (Number(row.total_deposit) || 0), 0), 2)}
                </span>
              </div>
              <div>
                <span className="text-gray-600">Total Withdrawals:</span>
                <span className="ml-2 font-semibold text-red-600">
                  {formatNumber(data.reduce((sum, row) => sum + (Number(row.total_withdrawal) || 0), 0), 2)}
                </span>
              </div>
              <div>
                <span className="text-gray-600">Net Balance:</span>
                <span className={`ml-2 font-semibold ${data.reduce((sum, row) => sum + (Number(row.net) || 0), 0) >= 0 ? 'text-green-600' : 'text-red-600'}`}>
                  {formatNumber(data.reduce((sum, row) => sum + (Number(row.net) || 0), 0), 2)}
                </span>
              </div>
            </div>
          </div>
