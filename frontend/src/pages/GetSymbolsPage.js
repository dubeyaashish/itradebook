...
const GetSymbolsPage = () => {
  ...
  useEffect(() => {
    ...
    newSocket.on('connect', () => {
      console.log('Connected to WebSocket for symbols data');
    });
    ...
    newSocket.on('trading_data_update', (updatedData) => {
      console.log('Received trading data update:', updatedData);
      setData(prevData => {
  ...
  const fetchSymbols = useCallback(async () => {
    try {
      ...
      const response = await axios.get('/api/getsymbols/symbols', {
  ...
  const fetchData = useCallback(async () => {
    try {
      setLoading(true);
      const token = localStorage.getItem('token');
      const response = await axios.get('/api/getsymbols/trading-data', {
        headers: { Authorization: `Bearer ${token}` }
      });
  ...
  if (loading) {
    return (
      <div className="page-container">
        <PageHeader 
          title="Symbol Trading Data" 
          subtitle="Live trading data for all symbols"
        />
        ...
        <div className="data-stats">
          <div className="flex justify-between items-center">
            ...
            <div className="text-sm text-gray-500">
              Live trading data - {new Date().toLocaleDateString()}
            </div>
          </div>
        </div>
