// frontend/src/pages/EodCustomerDataPage.js
...
const EodCustomerDataPage = () => {
  ...
  const fetchSymbols = useCallback(async () => {
    try {
      ...
      const response = await axios.get('/api/eod-customer-data/symbols', {
  ...
  const fetchData = useCallback(async () => {
    try {
      ...
      const response = await axios.get(`/api/eod-customer-data?${params}`, {
        headers: { Authorization: `Bearer ${token}` }
      });
      ...
      if (response.data.success) {
        ...
        setError('Failed to fetch EOD customer data');
      }
    } catch (error) {
      console.error('Error fetching data:', error);
      setError(error.response?.data?.error || 'Failed to fetch EOD customer data');
    } finally {
      setLoading(false);
    }
  ...
  if (loading) {
    return (
      <div className="page-container">
        <PageHeader 
          title="EOD Customer Data" 
          subtitle="End-of-day customer position data"
        />
        <div className="loading-container">
          <div className="loading-spinner">
            <div className="spinner"></div>
            <p>Loading EOD customer data...</p>
          </div>
        </div>
      </div>
    );
    ...
    <div className="page-container">
      ...
      <div className="data-container">
        <div className="table-header">
          <h3>EOD Customer Data ({formatNumber(pagination.total, 0)} records)</h3>
        </div>
        
        {data.length === 0 ? (
          <div className="no-data-message">
            No EOD customer data found for the selected filters.
          </div>
        ) : (
          <div className="table-responsive">
            <table className="data-table">
              <thead>
                <tr>
                  <th>Symbol</th>
                  <th>Total Buy Size</th>
                  <th>Total Sell Size</th>
            ...
            <div className="grid grid-cols-1 md:grid-cols-4 gap-4 text-sm">
              ...
              <div>
                ...
                <span className="ml-2 font-semibold text-green-600">
                  {formatNumber(data.reduce((sum, row) => sum + (row.total_buy_size || 0), 0), 2)}
                </span>
              </div>
              <div>
                <span className="text-gray-600">Total Sell Volume:</span>
                <span className="ml-2 font-semibold text-red-600">
                  {formatNumber(data.reduce((sum, row) => sum + (row.total_sell_size || 0), 0), 2)}
                </span>
              </div>
              <div>
...
export default EodCustomerDataPage;
